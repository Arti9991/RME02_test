...
#define NPC_11	 10   // DISP_IO, позиция 190 
#define NPC_12	 11   // 
#define NPC_13	 12   // 
#define NPC_14	 13   // 
#define NPC_15	 14   // 
...
#define SIZE_RF_CONTROL          72  // размер управляющего слова ctrl структуры HLItoIPCclass.control для упраления работой DISP_IO через DISP_SI
...
#pragma  message ("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
#endif
#define RF_TIME_PULSE_TEST			(RF_TIME_PULSE_COMMON + SIZE_RF_TIME_PULSE_COMMON) // 0x108 : время такта задаваемое программой timer.exe в 0.1мкс 
#define RF_TIME_WAIT_CIRS			 (RF_TIME_PULSE_TEST + SIZE_RF_TIME_PULSE_TEST)     // 0x110 : задержка в 0.1мкс для запуска процедур ЦИРСа
#define RF_IDENTIFIER			     (RF_TIME_WAIT_CIRS + SIZE_RF_WAIT_CIRS)            // 0x118 : идентификационный код первого запуска
#define RF_CONTROL			        (RF_IDENTIFIER + SIZE_RF_IDENTIFIER)               // 0x128 : смещение управляющего слова ctrl структуры HLItoIPCclass.control для упраления работой DISP_IO через DISP_SI

#define RF_STATUS_DISP_1     0x200                                   // 0x200

#if RF_CONTROL + SIZE_RF_CONTROL >  RF_STATUS_DISP_1
...
#define RES2	       	 	 10   // В базе данных PCADS06RTM
#define PCADS_v7K		     11   // В базе данных PCADSv7K
#define RES3						      12   // В базе данных PCADS06RK
#define PCI_1753								13   // В базе данных PCI-1753
#define RES4 											14   // В базе данных Astra
...
#define RES5            28   // В базе данных не объявлен !!!
#define pciSPI		        29   // В базе данных SPI
#define RS485_MTVS		    30   // В базе данных MTVS
#define RES6            31   // В базе данных не объявлен !!!
// ..................................................
...
struct ID_PCADSv7RK		//Структура подходит для адаптеров типа PCADSv7RKin и PCADSv7RKout
{
	...
	ULONG Line : 4;	    		// Номер линии передачи данных [0..6] - входные, [7..13] - выходные  (бит 11-14)
	ULONG bit15_23 : 9;	 	// Заполнение разрядов (биты 15-23)
	ULONG NumDevice : 3; 	// Порядковый номер адаптера данного типа в данном РС(биты 24-26)
	ULONG TypeDevice : 6;	// Тип адаптера (биты 27-32)
...
{
	ULONG InOut : 1;	// признак входной-1/выходной-0 (бит 1)
	ULONG IsInterrupt : 1;	// является параметр адресом для прерывания. 
	ULONG Service: 2;	// Параметр, по которому получается сервисная информация и помещается в поле Data:
	//1 - информация о прерываниях.
	//регистр задействованных прерываний, своими битами(с 0 по 15 - ый) показывает, какие прерывания будут задействованы на данном узле.  
	//(– регистр вектора прерываний, своими битами(с 16 по 31 - ый) показывает, какое прерывание было сгенерировано последним;
	//Если данный бит находится в единице, и InOut = 1, то всё остальное не учитывается
	//2 - Заполнение проходного буфера
	//3 - вывод счетчика пакетов
	ULONG TypePack : 5;	// Способ упаковки данных (биты 5-9)
	// ----------------------------------------------------------------------------------------------
	// TypePack   Тип            Размер     Описание
	// ----------------------------------------------------------------------------------------------
	// 0			 unknow							    	4					при упаковке использовать поля LoBit,HiBit,SignBit1,SignBit2,NumLowBit)
	...
	// 14				float								4					none 3.4E +/- 38 (7 digits)
	// 15				double								8					none 1.7E +/- 308 (15 digits)
	// 16				long double							8					none 1.2E +/- 4932 (19 digits)
	// ----------------------------------------------------------------------------------------------
	ULONG bit10_12 : 3;	// (биты 10-12) резерв
	ULONG irq : 4;	// (биты 13-16) номер прерывания [0..15]. (всего возможно 16 прерываний)
	ULONG bit17_26 : 10;	// (биты 17-26) резерв
	ULONG TypeDevice : 6;	// Тип адаптера = 21 (биты 27-32)
	ULONG Offset : 24;	// смещение параметра в памяти (биты 1-24)
	ULONG bit28 : 4;	// резерв (бит 25-28). 
	ULONG NumPC : 4;	// Номер компьютера (биты 29-32)
	union
	{
		//UINT64 u64bit; //Значение параметра 64bit. Раскоментировать, когда перейдем на 64 бит ос и структура glob растянется до 128 бит
		float f32bit;	 // Значение параметра 32bit
		UINT32 u32bit;	 // Значение параметра 32bit или данные для прерывания, если DestNId = 1...255
		USHORT u16bit;	 // Значение параметра 16bit
		UCHAR u8bit;	 // Значение параметра 8bit
	} Data;
};
...
union ID_Parameter
{
	...
	ID_PCADSv7K		     PCADSv7K;		 // определение параметра для PCADSv7 комутатора
	ID_iMKIO		       iMKIO;	       // определение параметра для iMKIO_1
	ID_RMe_2					 RMe_2;				 // определение параметра для RMe_2
};
